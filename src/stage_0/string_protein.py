...
def get_string(
    string_protein_path:str
):
    #load string_protein
    string_protein = utils.read_csv(
        string_protein_path,
        delimiter="\t",
        compression="gzip"
    ).select(["#string_protein_id", "preferred_name"]).rename_columns([
    ...
    string_protein = string_protein.drop(["string_id"]).append_column("string_id", string_id)
